...
pub(super) fn group_state_body(record: &GroupStateRecord) -> Result<Vec<u8>> {
    serde_json::to_vec(record).context("encoding consumer-group state")
}
...
pub(super) fn decode_group_state(body: &[u8]) -> Result<GroupStateRecord> {
    serde_json::from_slice(body).context("decoding consumer-group state")
}
