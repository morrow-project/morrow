...
test -s "$secret_dir/broker-admin-token" || openssl rand -hex 32 > "$secret_dir/broker-admin-token"
test -s "$secret_dir/broker-cluster-token" || openssl rand -hex 32 > "$secret_dir/broker-cluster-token"

if test ! -s "$secret_dir/broker-client-public-key"; then
  ...
  openssl pkey -in "$secret_dir/local-client-key.pem" -pubout -outform DER \
    | tail -c 32 | xxd -p -c 64 > "$secret_dir/broker-client-public-key"
fi

if test -e "$secret_dir/broker-ca-cert.pem"; then
...
openssl req -x509 -new -key "$work_dir/ca-key.pem" -days 3650 \
  -subj "/CN=broker-compose-ca" -addext "basicConstraints=critical,CA:TRUE" \
...
while test "$node" -le 3; do
  ...
  openssl req -new -key "$work_dir/node-$node-key.pem" \
    -subj "/CN=broker-$node" -out "$work_dir/node-$node.csr"
  printf 'subjectAltName=DNS:broker-%s,DNS:localhost,IP:127.0.0.1\nextendedKeyUsage=serverAuth,clientAuth\n' \
  ...
  mv "$work_dir/node-$node-key.pem" "$secret_dir/broker-node-$node-key.pem"
  mv "$work_dir/node-$node-cert.pem" "$secret_dir/broker-node-$node-cert.pem"
  node=$((node + 1))
done
mv "$work_dir/ca-cert.pem" "$secret_dir/broker-ca-cert.pem"
